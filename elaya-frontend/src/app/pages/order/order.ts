...
@Component({
  ...
  styles: [`
    ...
    .order-recap h2 { color: var(--gold); font-size: 1.3rem; border-bottom: 1px solid #333; padding-bottom: 1rem; }
    .mini-item { display: flex; justify-content: space-between; margin-bottom: 0.8rem; color: #ccc; }
    .total-row { display: flex; justify-content: space-between; border-top: 1px solid #333; padding-top: 1rem; font-size: 1.2rem; font-weight: bold; color: var(--gold); }

    .order-form-wrapper { background: var(--card-bg); padding: 2.5rem; border: 1px solid #333; border-top: 4px solid var(--gold); }
    ...
    .submit-btn { width: 100%; padding: 1.2rem; background: var(--gold); color: black; border: none; font-weight: bold; margin-top: 1.5rem; cursor: pointer; transition: 0.3s; }
    .submit-btn:hover:not(:disabled) { background: white; }
    .submit-btn:disabled { background: #333; cursor: not-allowed; }
    .error-msg { color: var(--red-wax); margin-top: 1rem; }

    /* --- RESPONSIVE --- */
    @media(max-width: 800px) {
      .order-layout { grid-template-columns: 1fr; }
      .form-row { grid-template-columns: 1fr; }
      .order-form-wrapper { padding: 1.5rem; }
    }
  `]
})
